...
import { diffInDays } from "../utils/diffInDays.js";
...
🔴 Ocupados: ${
        members.cache.filter((member) => member.presence?.status === "dnd").size
      }
⚫ Offline: ${
        members.cache.filter((member) => member.presence?.status === undefined)
          .size
      }
`,
    })
